...
from ansible_collections.equinix.cloud.plugins.module_utils.metal import AnsibleMetalModule
...
def act_on_project(target_state, module):
    ...
    first_1k_projects = module.metal_conn.find_projects(per_page=1000, exclude=['members']).projects
    if given_id:
        matching_projects = [
            p for p in first_1k_projects if given_id == p.id]
    else:
        matching_projects = [
            p for p in first_1k_projects if given_name == p.name]
    ...
    if target_state == 'present':
        if len(matching_projects) == 0:
            ...
            if not org_id:
                createParams = equinixmetalpy.models.ProjectCreateFromRootInput(
                    name=given_name,
                    payment_method_id=payment_method,
                    customdata=custom_data,
                )
                new_project = equinixmetalpy.create_project(createParams)
            else:
                pass
...
def main():
    ...
    if not HAS_METAL_SDK:
        module.fail_json(msg = 'equinixmetal required for this module')
